...
通过 **Wrangler CLI** 将 Worker 部署到 Cloudflare，仅需 5 分钟。部署完成后，在 Cloudflare Dashboard 中设置 Secrets 和 Variables 即可运行。

**需要配置的内容：**

| 类型 | 名称 | 说明 |
|------|------|------|
| Secret | `OKX_API_KEY` | OKX API Key（建议只读权限） |
| Secret | `OKX_API_SECRET` | OKX API Secret |
| Secret | `OKX_API_PASSPHRASE` | OKX API Passphrase |
| Variable | `OKX_DEMO` | `true` = 模拟盘，`false` = 实盘 |

```bash
# 1. 克隆仓库并进入目录
git clone https://github.com/jjbb013/okx-multi-trading-web-demo.git
...
# 7. 获取访问地址
# 部署成功后终端会显示类似：
# https://okx-monitor-01.your-account.workers.dev
```
...
- **GitHub Actions 自动部署**：配置后推送即自动部署
...
Deploy the Worker to Cloudflare using **Wrangler CLI** in about 5 minutes. After deployment, configure Secrets and Variables in the Cloudflare Dashboard.

**Configuration required:**

| Type | Name | Description |
|------|------|-------------|
| Secret | `OKX_API_KEY` | OKX API Key (read-only recommended) |
| Secret | `OKX_API_SECRET` | OKX API Secret |
| Secret | `OKX_API_PASSPHRASE` | OKX API Passphrase |
| Variable | `OKX_DEMO` | `true` = demo, `false` = live |
...
- **GitHub Actions auto-deploy**: Push to deploy after configuration
